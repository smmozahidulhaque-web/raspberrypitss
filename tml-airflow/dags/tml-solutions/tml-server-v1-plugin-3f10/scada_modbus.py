...
def modbus_read_loop(scada_cfg, interval_s, callback_url, max_reads, fields, scaling, start_address, sendtotopic, job_id, VIPERTOKEN, VIPERHOST, VIPERPORT,args,createvariables=""):
        ...
        while not sg.read_job['stop']: #True:
            ...
            try:
                ...
                if isinstance(regs, list) and len(regs) > 0:
                    ...
                    payload = {
                        k: v for k, v in locals().items() if k in ['scada_cfg', 'interval_s', 'callback_url', 'max_reads', 'fields', 'scaling', 'start_address','sendtotopic']
                    }
                    payload.update({
...
def payload_to_osdu_dynamic(processed_payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    config_keys = {
        ...
        'createvariables', 'fields', 'scaling','sendtotopic'
    }
