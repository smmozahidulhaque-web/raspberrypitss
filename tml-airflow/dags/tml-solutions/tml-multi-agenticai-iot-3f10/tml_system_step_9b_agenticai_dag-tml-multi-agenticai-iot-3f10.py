...
def startpgptcontainer():
      ...
      time.sleep(10)
      if os.environ['TSS'] == "1":       
          buf = "docker run -d -p {}:{} --net=host -v /var/run/docker.sock:/var/run/docker.sock:z {} --env OLLAMA_LOAD_TIMEOUT=30m0s --env PORT={} --env TSS=1 --env GPU=0 --env COLLECTION={} --env WEB_CONCURRENCY={} --env TOKENIZERS_PARALLELISM=false --env temperature={} --env LLAMAMODEL=\"{}\" --env mainembedding=\"{}\" --env OLLAMASERVERPORT=\"{}\" {}".format(mainport,mainport,localmodels,mainport,collection,concurrency,temperature,mainmodel,mainembedding,ollamaserver,ollamacontainername)
      else:
          buf = "docker run -d -p {}:{} --net=host -v /var/run/docker.sock:/var/run/docker.sock:z {} --env OLLAMA_LOAD_TIMEOUT=30m0s --env PORT={} --env TSS=0 --env GPU=0 --env COLLECTION={} --env WEB_CONCURRENCY={} --env TOKENIZERS_PARALLELISM=false --env temperature={} --env LLAMAMODEL=\"{}\" --env mainembedding=\"{}\" --env OLLAMASERVERPORT=\"{}\" {}".format(mainport,mainport,localmodels,mainport,collection,concurrency,temperature,mainmodel,mainembedding,ollamaserver,ollamacontainername)
